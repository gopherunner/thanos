...
func (c cacheItem) size() uint64 {
	...
	case cacheKeyPostings:
		...
		return sliceHeaderSize + 2*16 + uint64(len(k.Value)+len(k.Name))
	case cacheKeySeries:
		return sliceHeaderSize + 8 // ULID + uint64
	}
	return 0
}
...
func NewIndexCache(logger log.Logger, reg prometheus.Registerer, opts Opts) (*IndexCache, error) {
	...
	level.Info(logger).Log(
		...
		"setTimeout", c.setTimeout.String(),
...
func (c *IndexCache) onEvict(key, val interface{}) {
	...
	c.totalCurrentSize.WithLabelValues(string(k)).Sub(float64(entrySize + key.(cacheItem).size()))
...
func (c *IndexCache) SetPostings(b ulid.ULID, l labels.Label, v []byte) {
	...
	cv := make([]byte, len(v))
	copy(cv, v)
	item := cacheItem{b, cacheKeyPostings(l)}
	c.lru.Add(item, cv)

	c.added.WithLabelValues(cacheType).Inc()
	c.currentSize.WithLabelValues(cacheType).Add(float64(entrySize))
	c.totalCurrentSize.WithLabelValues(cacheType).Add(float64(entrySize + item.size()))
...
func (c *IndexCache) SetSeries(b ulid.ULID, id uint64, v []byte) {
	...
	cv := make([]byte, len(v))
	copy(cv, v)
	item := cacheItem{b, cacheKeySeries(id)}
	c.lru.Add(item, cv)
